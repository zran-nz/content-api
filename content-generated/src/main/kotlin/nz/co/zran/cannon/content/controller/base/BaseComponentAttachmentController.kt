...
@Transactional
abstract class BaseComponentAttachmentController : BaseController<ComponentAttachment, Long>() {

    @GraphRender("componentAttachment")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentAttachment, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentAttachment, request: HttpServletRequest): ResponseEntity<*> {
