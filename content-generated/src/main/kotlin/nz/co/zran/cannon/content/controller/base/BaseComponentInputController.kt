...
@Transactional
abstract class BaseComponentInputController : BaseController<ComponentInput, Long>() {

    @GraphRender("componentInput")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentInput, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentInput, request: HttpServletRequest): ResponseEntity<*> {
