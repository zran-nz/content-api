...
@Transactional
abstract class BaseResourceComponentController : BaseController<ResourceComponent, Long>() {

    @GraphRender("resourceComponent")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ResourceComponent, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ResourceComponent, request: HttpServletRequest): ResponseEntity<*> {
