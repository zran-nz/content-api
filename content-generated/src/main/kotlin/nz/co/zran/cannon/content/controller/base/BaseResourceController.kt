...
@Transactional
abstract class BaseResourceController : BaseController<Resource, Long>() {

    @GraphRender("resource")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: Resource, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: Resource, request: HttpServletRequest): ResponseEntity<*> {
