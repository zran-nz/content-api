...
@Transactional
abstract class BaseResourceTemplateController : BaseController<ResourceTemplate, Long>() {

    @GraphRender("resourceTemplate")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ResourceTemplate, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ResourceTemplate, request: HttpServletRequest): ResponseEntity<*> {
