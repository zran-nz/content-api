...
@Transactional
abstract class BaseComponentSelectController : BaseController<ComponentSelect, Long>() {

    @GraphRender("componentSelect")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentSelect, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentSelect, request: HttpServletRequest): ResponseEntity<*> {
