...
@Transactional
abstract class BaseComponentQuizChoiceController : BaseController<ComponentQuizChoice, Long>() {

    @GraphRender("componentQuizChoice")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentQuizChoice, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentQuizChoice, request: HttpServletRequest): ResponseEntity<*> {
