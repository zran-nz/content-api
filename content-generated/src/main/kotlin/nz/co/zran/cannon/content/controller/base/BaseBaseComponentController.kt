...
@Transactional
abstract class BaseBaseComponentController : BaseController<BaseComponent, Long>() {

    @GraphRender("baseComponent")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: BaseComponent, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: BaseComponent, request: HttpServletRequest): ResponseEntity<*> {
