...
@Transactional
abstract class BaseResourceSelectController : BaseController<ResourceSelect, Long>() {

    @GraphRender("resourceSelect")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ResourceSelect, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ResourceSelect, request: HttpServletRequest): ResponseEntity<*> {
