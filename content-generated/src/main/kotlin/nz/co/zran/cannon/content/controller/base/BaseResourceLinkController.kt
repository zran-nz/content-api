...
@Transactional
abstract class BaseResourceLinkController : BaseController<ResourceLink, Long>() {

    @GraphRender("resourceLink")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ResourceLink, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ResourceLink, request: HttpServletRequest): ResponseEntity<*> {
