...
@Transactional
abstract class BaseResourceTextareaController : BaseController<ResourceTextarea, Long>() {

    @GraphRender("resourceTextarea")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ResourceTextarea, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ResourceTextarea, request: HttpServletRequest): ResponseEntity<*> {
