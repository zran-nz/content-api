...
@Transactional
abstract class BaseResourceAttachmentController : BaseController<ResourceAttachment, Long>() {

    @GraphRender("resourceAttachment")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ResourceAttachment, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ResourceAttachment, request: HttpServletRequest): ResponseEntity<*> {
