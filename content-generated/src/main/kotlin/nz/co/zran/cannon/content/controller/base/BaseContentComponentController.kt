...
@Transactional
abstract class BaseContentComponentController : BaseController<ContentComponent, Long>() {

    @GraphRender("contentComponent")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ContentComponent, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ContentComponent, request: HttpServletRequest): ResponseEntity<*> {
