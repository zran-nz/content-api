...
@Transactional
abstract class BaseComponentRadioController : BaseController<ComponentRadio, Long>() {

    @GraphRender("componentRadio")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentRadio, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentRadio, request: HttpServletRequest): ResponseEntity<*> {
