...
@Transactional
abstract class BaseResourceChoiceController : BaseController<ResourceChoice, Long>() {

    @GraphRender("resourceChoice")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ResourceChoice, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ResourceChoice, request: HttpServletRequest): ResponseEntity<*> {
