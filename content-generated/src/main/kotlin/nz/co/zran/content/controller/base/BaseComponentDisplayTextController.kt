...
@Transactional
abstract class BaseComponentDisplayTextController : BaseController<ComponentDisplayText, Long>() {

    @GraphRender("componentDisplayText")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentDisplayText, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentDisplayText, request: HttpServletRequest): ResponseEntity<*> {
