...
@Transactional
abstract class BaseComponentTextareaController : BaseController<ComponentTextarea, Long>() {

    @GraphRender("componentTextarea")
    @GetMapping
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentTextarea, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentTextarea, request: HttpServletRequest): ResponseEntity<*> {
