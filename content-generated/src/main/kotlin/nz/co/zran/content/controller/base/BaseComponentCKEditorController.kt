...
@Transactional
abstract class BaseComponentCKEditorController : BaseController<ComponentCKEditor, Long>() {
    ...
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: ComponentCKEditor, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
        return super.saveOne(input, request, uriComponent)
    }

    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: ComponentCKEditor, request: HttpServletRequest): ResponseEntity<*> {
